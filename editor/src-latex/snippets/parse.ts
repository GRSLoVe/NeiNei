...
/**
 * Variables de snippet ya cargadas como objeto (p. ej. import estático).
 * Evita `import(data:…)` en tiempo de ejecución, que falla con CSP o en algunos navegadores.
 */
export function parseSnippetVariablesFromObject (raw: unknown): SnippetVariables {
	if (raw === null || typeof raw !== "object" || Array.isArray(raw)) {
		throw "Cannot parse an array as a variables object";
	}
	const rawSnippetVariables = raw as Record<string, string>;
...
export async function parseSnippetVariables (snippetVariablesStr: string) {
	...
	return parseSnippetVariablesFromObject(rawSnippetVariables);
}

/** Snippets ya cargados como array (p. ej. `export default` empaquetado). */
export function parseSnippetsFromArray (rawSnippets: unknown, snippetVariables: SnippetVariables): Snippet[] {
...
export async function parseSnippets (snippetsStr: string, snippetVariables: SnippetVariables) {
	...
	return parseSnippetsFromArray(rawSnippets, snippetVariables);
}
