...
export const keyboardEventPlugin = ViewPlugin.fromClass(class {
	...
	onKeydown(event: KeyboardEvent, view: EditorView) {
		try {
			...
			const success =
				safeHandleKeydown(event.key, event.ctrlKey || event.metaKey, isComposing(view, event), view) ||
				runScopeHandlers(view, event, "latex-suite");

			if (success) event.preventDefault();
		} catch (e) {
			console.error("latex-suite keydown:", e);
		}
	}
}, {
	eventHandlers: {
...
export const onInput = (
	...
	try {
		...
		// En CM6 este valor puede aparecer en rutas raras; bloquear la inserción deja el editor mudo.
		if (text === "\0\0") return false;
		if (text.length == 1 && lastKeyboardEvent) {
			let key = text;
			if (key === "\t") key = "Tab";
			const success = safeHandleKeydown(
				key,
		...
		console.error("latex-suite onInput:", e);
		return false;
	}
};

function safeHandleKeydown (key: string, ctrlKey: boolean, isIME: boolean, view: EditorView): boolean {
	try {
		return handleKeydown(key, ctrlKey, isIME, view);
	} catch (e) {
		console.error("latex-suite handleKeydown:", e);
		return false;
	}
}
